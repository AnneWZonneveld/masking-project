...
def logit_model(data, features, exp = True, thres = 0):
    ...
    for i in range(len(layers_oi)):
        ...
        features = np.reshape(X[layer],(X[layer].shape[0], -1)) # flatten
        features_pca = pca.fit_transform(features)
        X_transform[layer] = features_pca

    # n_components = 100
    # pca = PCA(n_components=n_components) 
    # features = np.reshape(X['all'],(X['all'].shape[0], -1)) # flatten
    # features_pca = pca.fit_transform(features)
    # X_transform[layer] = features_pca
    ...
    if exp == True:
        ...
        pkl.dump(X2, file)
        file.close()
        file_path = os.path.join(wd, 'analysis/stats/X_transform')
        file = open(file_path, 'wb')
        pkl.dump(X_transform, file)
        file.close()
    else:
    ...
    rel_drops = []
    X_transform = {}
    thresholder = VarianceThreshold(thres)
    fig, ax = plt.subplots(1,n_layers + 1, figsize=(14,3))
    for i in range(n_layers + 1):
        layer = layers_oi[i]
        var = np.var(X[layer], axis=0)
        min = np.min(var)
        mins.append(min)
        max = np.max(var)
        maxs.append(max)
        ...
        data = data.rename(columns={0: 'var'})
        # sns.histplot(data=data, x='var', ax=ax[i], stat='probability', bins=40)
        sns.histplot(data=data, x='var', ax=ax[i], bins=40)
        ax[i].set_title(f"{layer}")
        ax[i].set_xlabel('')
        ax[i].set_ylabel('')

        c_X = X[layer]
        c_X_high_variance = thresholder.fit_transform(c_X)
        X_transform[layer] = c_X_high_variance
        drop = c_X.shape[1] - c_X_high_variance.shape[1]
        rel_drop = drop/c_X.shape[1]
        rel_drops.append(rel_drop)
        drops.append(drop)

    fig.supxlabel('Feature variance')
    fig.supylabel('Count')
    fig.tight_layout()
    if exp == True:
        image_path = os.path.join(wd, 'analysis/var_X.png')
    else:
        image_path = os.path.join(wd, 'analysis/var_X_random.png')
    fig.savefig(image_path)
    print(f"max variance: {maxs}")
    print(f"min variance: {mins}")
    plt.clf()

    # Plot var distribution after feature threshold
    mins = []
    maxs = []
    fig, ax = plt.subplots(1,n_layers + 1, figsize=(14,3))
    for i in range(n_layers + 1):
        layer = layers_oi[i]
        var = np.var(X_transform[layer], axis=0)
        min = np.min(var)
        mins.append(min)
        max = np.max(var)
        maxs.append(max)
        ...
        data = pd.DataFrame(var)
        data = data.rename(columns={0: 'var'})
        # sns.histplot(data=data, x='var', ax=ax[i], stat='probability', bins=20)
        sns.histplot(data=data, x='var', ax=ax[i], bins=20)
        ax[i].set_title(f"{layer}")
        ax[i].set_xlabel('')
        ax[i].set_ylabel('')
    ...
    fig.supylabel('Count')
    fig.tight_layout()
    image_path = os.path.join(wd, 'analysis/var_X_transform.png')
    fig.savefig(image_path)
    print(f"max variance: {maxs}")
    print(f"min variance: {mins}")
    plt.clf()
    ...
    # Zoom in layer 4-all
    c_layers = ['layer1', 'layer4', 'layer5', 'all']
    ...
    for i in range(len(c_layers)):
        ...
        data = pd.DataFrame(var)
        data = data.rename(columns={0: 'var'})
        # sns.histplot(data=data, x='var', ax=ax[i], stat='probability', binwidth=0.0001)
        sns.histplot(data=data, x='var', ax=ax[i], binwidth=0.0001)
        ax[i].set_title(f"{layer}")
        ax[i].set_xlabel('')
        ax[i].set_ylabel('')
        # ax[i].set_ylim([0.0, 0.4])
        ax[i].set_xlim([0.0, 0.005])
    
    fig.supxlabel('Feature variance')
    fig.supylabel('Count')
    fig.tight_layout()
    image_path = os.path.join(wd, 'analysis/var_X_transform_zoom.png')
    fig.savefig(image_path)
    print(f"max variance: {maxs}")
    print(f"min variance: {mins}")
    plt.clf()
    ...
    # lr = LinearRegression()
    # # clf = make_pipeline(VarianceThreshold(0.001), preprocessing.StandardScaler(), lr)
    # # clf = make_pipeline(VarianceThreshold(0.0005), preprocessing.StandardScaler(), lr)
    # clf = make_pipeline(VarianceThreshold(thres), preprocessing.StandardScaler(), lr)
    ...
    #     # file_path = os.path.join(wd, f'analysis/stats/model_fit_{layer}')
    #     # file = open(file_path, 'wb')
    #     # pkl.dump(clf, file)
    #     # file.close()
    ...
    for i in range(n_layers):
        ...
        preds[:, i] = cross_val_predict(clf, X_transform[layer], y, cv=n_mask_trials)
        # file_path = os.path.join(wd, f'analysis/stats/model_fit_{layer}')
        # file = open(file_path, 'wb')
        # pkl.dump(clf, file)
        # file.close()
    print(f"CV all layer model")
    preds[:, i + 1] = cross_val_predict(clf, X_transform['all'], y, cv=n_mask_trials)
    
    # file_path = os.path.join(wd, 'analysis/stats/model_fit_all')
    # file = open(file_path, 'wb')
    # pkl.dump(clf, file)
    # file.close()
    ...
    if exp == True:
        np.save(os.path.join(file_path, f"lr_predictions_v{thres}.npy"), preds)
    else: 
        np.save(os.path.join(file_path, f"lr_predictions_random_v{thres}.npy"), preds)
    ...
    if exp == True:
        image_path = os.path.join(wd, f'analysis/MAEs-layers_v{thres}.png')
    else:
        image_path = os.path.join(wd, f'analysis/MAEs-layers_random_v{thres}.png')
    plt.savefig(image_path)
    plt.clf()
    ...
    plt.tight_layout()
    if exp == True:
        image_path = os.path.join(wd, f'analysis/r2-layers_v{thres}.png')
    else:
        image_path = os.path.join(wd, f'analysis/r2-layers_random_v{thres}.png')
...
def PLS_model():
    ...
    # Set up pipeline
    lr = PLSRegression(n_components=100)
    clf = make_pipeline(VarianceThreshold(thres), preprocessing.StandardScaler(), lr)
    ...
    print(f"CV all layer model")
    pred = cross_val_predict(clf, X['all'], y, cv=n_mask_trials)
    preds[:, i + 1] =  np.squeeze(pred)
    ...
    if exp == True:
        np.save(os.path.join(file_path, f"PLS_predictions_v{thres}.npy"), preds)
    else: 
        np.save(os.path.join(file_path, f"PLS_predictions_random_v{thres}.npy"), preds)
    ...
    if not os.path.exists(file_path):
            os.makedirs(file_path)
    MAE_df.to_csv(os.path.join(file_path, 'PLS_MAE_per_layer.csv'), index=False)
    ...
    if exp == True:
        image_path = os.path.join(wd, f'analysis/PLS_MAEs-layers_v{thres}.png')
    else:
        image_path = os.path.join(wd, f'analysis/PLS_MAEs-layers_random_v{thres}.png')
    plt.savefig(image_path)
    plt.clf()
    ...
    plt.tight_layout()
    if exp == True:
        image_path = os.path.join(wd, f'analysis/r2-layers_v{thres}_PLS.png')
    else:
        image_path = os.path.join(wd, f'analysis/r2-layers_random_v{thres}_PLS.png')
...
def model_per_mask():
    ...
    np.save(os.path.join(file_path, "MAE_per_mask_fitted.npy"), preds)
    ...
    sns.despine(offset=10)
    xlabels = ['layer 1', 'layer 2', 'layer 3', 'layer 4', 'layer 5', 'all']
    # xlabels = ['layer 1', 'layer 2', 'layer 3', 'layer 4', 'layer 5']
    plt.xticks(np.arange(len(xlabels)),xlabels)
    plt.xlabel('')
    plt.tight_layout()
...
# Set up random model
# logit_model(bdata, random_features, exp=False, thres=0)

# Set up log regression model
logit_model(bdata, exp_features, exp=True, thres=0)
